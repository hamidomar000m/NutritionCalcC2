package gui;

import java.awt.Color;
import java.awt.Font;

public class Constants {

	/*
	 * GENERAL
	 */
	public static String VERSION = "V2.0";
	
	/*
	 * COLORS
	 */
	public static Color MIDGREEN = new Color(0, 139, 0);
	public static Color DARKGREEN = new Color(0, 100, 0);
	public static Color LIGHTGRAY = new Color(204, 204, 204);
	public static Color DARKGRAY = new Color(150, 150, 150);
	public static Color MAINBACKGROUND = new Color(240, 240, 240);
	
	/*
	 * FONTS
	 */
	public static Font PLAINTEXT = new Font("Century Gothic", Font.PLAIN, 14);
	public static Font LOGINTEXT = new Font("Century Gothic", Font.PLAIN, 18);
	public static Font BUTTONTEXT = new Font("Century Gothic", Font.PLAIN, 18);
	public static Font BUTTONTEXT2 = new Font("Century Gothic", Font.PLAIN, 16);
	public static Font BUTTONTEXTBOLD = new Font("Century Gothic", Font.BOLD, 20);
	public static Font BUTTONTEXTBOLD2 = new Font("Century Gothic", Font.BOLD, 18);
	public static Font HEADING = new Font("Century Gothic", Font.BOLD, 28);
	public static Font HEADING1 = new Font("Century Gothic", Font.BOLD, 20);
	
}
